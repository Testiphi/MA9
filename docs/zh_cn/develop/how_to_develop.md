...
（要求 `data/multiplayer_profile.json`）。两者的**优先级层级一致（显式配置 → 标记 → 无标记原有行为）**，
但**无标记时的候选起点顺序并不相同**，见下。
...
3. 无标记时的原有行为：候选起点顺序**两函数各自保留**：GUI 是 exe 所在目录 → 当前工作目录；
   Agent 是当前工作目录 → exe 所在目录 → 模块目录。按该顺序展开祖先，优先返回带 `config/garage.json`
   的账号根，否则退回首个带数据的目录。两函数的优先级层级一致，不代表这里的起点顺序相同。

**标记的语义是「主动隔离」**

查找按起点**顺序穷尽**，与层数无关：先穷尽第一个起点的整条祖先链（如 exe 所在目录及其全部上级），
再穷尽下一个起点（如 cwd 及其全部上级）；Agent 把模块目录留到最后。**不做并行、交替或同层优先的层序遍历。**
命中第一个标记即停止，不再向更外层查找。若该标记目录缺少本函数所需数据则抛 `FileNotFoundError`，
**不越过标记**去读另一个账号的数据。因此 exe 链上一个较深的标记会胜过 cwd 链上一个较浅的标记。
