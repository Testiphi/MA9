...
4. 每个 lane 的提示词按本文件第 9 节的模板写，不要自己发挥。
   ...
   当前不启动 GPT 子智能体；DeepSeek 席位由编排层生成完整提示词，
...
- 用户已改回 DeepSeek 人工中转；具体档位见 lanes.yaml。
...
### 0.1 当前模型政策（2026-09-22 用户再次调整）

- 总控保持 Astra，负责边界、证据、验收与合并；禁止自动启动 GPT 子智能体。
- 普通业务 owner 改为 DeepSeek V4.1 Flash high，经 WorkBuddy 用户复制提示词及回传文件。
- 有 reviewer 的 lane 使用另一个独立 DeepSeek high 只读上下文；owner 修复，总控独立本地验收。
- 机械资料整理可使用 DeepSeek low；一次一个 owner，04 暂停，06/07 等依赖。
...
- 默认同时一个 DeepSeek owner，交付后再交给独立 DeepSeek reviewer。
...
- **换模政策**：当前 DeepSeek high 人工中转，验收标准保持不变。
