...
- 用户已批准成本平衡政策：Terra 主力、Luna 窄任务、Sol 按需难题；具体档位见 lanes.yaml。
...
- **主要执行模型**：GPT-5.6 Terra。`duel-scan` 与擂台必需的 `vehicle` 修复用 high；
  `duel-defense` 与按需 `build` 维护用 medium，复杂问题升 high。
- **窄任务模型**：GPT-5.6 Luna。`nav` 仅明确的小修复用 high；日志/截图索引等辅助用 low/medium。
  共享判据变更、复杂生成器诊断转 Terra high。能用脚本完成的工作不额外派模型。
- **最难算法席**：`duel-attack` 按需使用 GPT-5.6 Sol high；max 仅用于有证据的具体算法瓶颈。
- **独立复核**：`lanes.yaml` 存在 `reviewer` 的 lane，在 owner 验收通过后必须由 GPT-5.6 Terra high
  做只读复核；复核者不得修代码，之后仍由 Astra 编排层决定是否合入。
- **外部辅助**：DeepSeek V4.1 Flash low/high 经 WorkBuddy 做独立、非阻塞的资料整理；
  使用用户已有额度，不要求新增订阅。GLM/Kimi 为备用，不默认分配业务 lane；MiniMax 渠道未定不得派活。
- `effort` 必须写供应商原生枚举，完整允许值见 lanes.yaml.model_tiers。
...
- GPT-6 Astra、GPT-5.6 Sol、GPT-5.6 Terra、GPT-5.6 Luna 通过 Codex 原生子智能体直接启动；
...
- 默认同时一个 owner，交付后再启动独立 reviewer；有明确收益时才并行一个 Luna/外部辅助。
...
- 子智能体提示词只传任务必需上下文。外部辅助给定输入清单、输出格式、结束条件，一包一次交付，
  不让关键路径等待外部整理；外部不修改契约、核心算法、共享判据或受控生成物。
...
- **子任务升档**：同一明确问题至多进行两轮针对性修复；仍失败就保存最小复现返回总控。
  Astra high 诊断后决定是否派 Sol high 的有界修复，不允许低成本模型无限重试。
- **max**：不是常驻默认值，只有具体算法瓶颈有证据时才考虑。
- **换模试点**：用户已批准 duel-scan 从原固定 max 改为 Terra high，duel-attack 改为 Sol high。
  用现有固定样例、边界和失败路径测试验证，保留 Terra high 独立复核；不因此降低验收标准。
