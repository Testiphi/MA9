...
- 到达等待采用双上限：最多120次采样，并用time.monotonic固定30秒截止（从开始到达等待起算，不因读到第一帧或异常重置截止）。间隔采用0.3秒。每次发起新采样前检查截止，避免超时后开始新采样；单次底层截图/OCR可能阻塞超过截止，必须如实说明非硬墙钟中断，不另加线程或强杀设备调用。
- 仍必须连续两次可信“车辆选择”才True，任何无效/异常重置连续计数，预算耗尽False。不能首帧即成功、不能复用前置/第一帧作第二帧，不能增加点击重试或自动返回。
- 常量/docstring写清新预算；回调总最大采样4+1+120=125（主流程05F前后与scan另计），顺利5。不能声称固定耗时30秒或全部125必执行，时间与次数先到者限制新采样。
...
3. 始终非到达页：快时钟场景最多120帧，时间截止场景到期不开始下一帧；边界0/30秒、时间前进与单次读取超时的处理有确定性测试，不真实sleep30秒。
...
## 用户补充后的当前裁决（替代旧20次/15秒要求）
用户观察：按钮按下后游戏会明显卡顿，多处加载较久。等待旧画面是允许的过渡状态，不是可重复点击的理由。以页面状态确认驱动推进，双上限现为最多120次+固定monotonic30秒，采样间隔0.3秒；防止旧20次预算在快速截图时约4秒就先耗尽，使名义15秒失效。实际单次截图/OCR耗时仍计入截止，无法中断底层调用，不承诺30秒硬返回。
保留整串标题/置信度/ROI和连续两帧守卫，不放宽识别以“容忍卡顿”。不要用固定sleep30秒代替轮询；一旦两帧确认立即推进。一次点击后未知/旧画面只继续观察，不补点。截止未确认就停止，不盲目继续。
额外必测：快截图持续旧页10秒以上、随后两帧到达且仍在30秒内，应成功，不能被次数上限提前卡住；模拟15~25秒才加载完；持续不加载到30秒停止；末秒只有一帧仍拒绝。使用fake monotonic/sleep，不真实等待。其他页面的统一等待治理只登记后续，不改本轮两个文件外的源码，不泛化为万能重试框架。
