...
准确构建源/预期起止HEAD：f074163fb77a163f76b73239a10842c03f07a90c
...
- E:/hzz/work/MA9/MA9-evidence/20260924-02G-f074163/：report.md、results.json、build/package/verify/smoke日志、hash清单、离线校验JSON、tmp/与本轮私有Python薄包装/核验脚本、build-root/、pyinstaller-cache/。
- E:/hzz/work/MA9/MA9-worktrees/duel-scan/build/user-test-slot-f074163/MA9-preview/：新包唯一根。
...
1. importlib加载本lane tools/build_agent.py（不执行main直到覆盖输出设置）。只把模块ROOT改为本证据/build-root，AGENT_DIR显式保持本lane/agent；确认main.py实际源为f074163。sys.argv只给工具名、不传--clean，然后调用main。现有工具内部PyInstaller --clean只针对本轮全新cache/work/spec目录；若宿主仍拦截停止上报，不去解除守卫/提升删除权限。所有输出路径实际resolve并检查在两授权目录内。目录存在不重试覆盖，失败保留，报告总控。
2. 使用本lane prepare_portable_preview.py的原main与完备性检查，通过私有包装覆盖：ROOT=本lane；SOURCE_UI=E:/hzz/work/MA9/install（只复用现有MFA UI/host，逐项hash，不复用其Agent/interface/config）；AGENT=本轮build-root/build/agent/win-x64/dist/ma9-agent；BASE=本lane/build/user-test-slot-f074163；DESTINATION=BASE/MA9-preview；TASKS仅{多人运行时_数据自检,对决_隔离单槽定位测试}。sys.argv仅工具名，不zip。运行前确认DESTINATION不存在，避免原工具删除分支。
3. 资源、5项runtime数据必须来自f074163 lane；若OCR二进制模型为未跟踪缓存缺失，明确列出所需文件与主仓库现存受信本地缓存来源/哈希，再复制到本次包资源，不写lane源码、不联网下载，不用旧资源整体覆盖。除明示缓存外，资源逐文件与lane一致。
...
7. 包根新建TEST-BUILD.json：完整source HEAD、版本v0.0.0-slot-locate-f074163、构建脚本/解释器与exe哈希、构建根、源界面与包界面变换清单、marked-root阶段/私有绑定阶段、device_test=not_run、choose=false。绝不称整个包与source逐字节同一，因为interface明确过滤和child_exec变换。
...
复用而非重跑：总控Agent265/tools30 exit0、05H新增pipeline+变更interface两项schema exit0，旧资源历史schema复用，native OCR18/18；本轮包界面经过过滤，应对最终interface及新增pipeline实跑schema（可用本轮tmp小资源目录+源validate_schema工具，旧未变资源复用），不能说全27重跑。不要跑npm或重建大分片。
...
## 本轮新增验包重点（02G替代旧02F包，不覆盖旧包）
本次源f074163只在已验cf8815b之上修改入页等待。必读E:/hzz/work/MA9/MA9-evidence/20260924-05G1R-wait-review/results.json与20260924-05G1-integration/results.json，Agent265/tools30最终exit0为历史复用，不重跑；新包仍未实机。
旧包位于build/user-test-slot-cf8815b/MA9-preview，及其debug、旧02F证据一律保留，不替换其exe/配置/日志。新包路径build/user-test-slot-f074163/MA9-preview；新私有证据20260924-02G-f074163；旧包装脚本可读参考，不直接运行旧main，不写旧目录。
原02F总控验包有两项必须落实：OCR文件须独立与声明的host缓存来源逐文件hash比较，不能把包内OCR本身作为expected；负对照compile使用与实际模块一致的文件basename，避免旧文件重命名产生metadata差异冒充业务差异。
额外负对照：git show cf8815b2268d07142b679b06272101e9f1b2191e:agent/ma9_agent/duel_slot_entry.py，与包内ma9_agent.duel_slot_entry比较必须有代码/常量差异，比较进程exit1（如总验证器用assert封装返回0，必须同时给出负对照子进程的exit1）。确认新模块包含120/.3/30.0及monotonic截止代码且与f074163递归一致，不能只grep常量字符串。
包interface的version本次明确设置v0.0.0-slot-locate-f074163，以便GUI区分，另在TEST-BUILD中记录version这项输出变换；这仅改新包产物，不改source interface。除原agent/controller/task三项之外仅允许version这一新差异，核验最终interface schema。上轮source和包均显示1.0.0的口径不沿用。
用户私有模板仍从20260924-05H-test-wiring/user-request.template.json读取，绑定新的绝对根，账号/slot1/Nevera/choose=false均不变。不复用旧包runtime_root。先原prepare完备性与标记，再私有config，与原说明一致。
交付注明“慢加载修复包”，不把15~25秒模拟等待测试称实机证明。回传后由总控验包再给用户重试步骤。
