...
准确构建源/预期起止HEAD：da22bf1bc844d762d75fb70977284dced7b09e11
...
- E:/hzz/work/MA9/MA9-evidence/20260924-02H-da22bf1/：report.md、results.json、build/package/verify/smoke日志、hash清单、离线校验JSON、tmp/与本轮私有Python薄包装/核验脚本、build-root/、pyinstaller-cache/。
- E:/hzz/work/MA9/MA9-worktrees/duel-scan/build/user-test-slot-da22bf1/MA9-preview/：新包唯一根。
...
1. importlib加载本lane tools/build_agent.py（不执行main直到覆盖输出设置）。只把模块ROOT改为本证据/build-root，AGENT_DIR显式保持本lane/agent；确认main.py实际源为da22bf1。sys.argv只给工具名、不传--clean，然后调用main。现有工具内部PyInstaller --clean只针对本轮全新cache/work/spec目录；若宿主仍拦截停止上报，不去解除守卫/提升删除权限。所有输出路径实际resolve并检查在两授权目录内。目录存在不重试覆盖，失败保留，报告总控。
2. 使用本lane prepare_portable_preview.py的原main与完备性检查，通过私有包装覆盖：ROOT=本lane；SOURCE_UI=E:/hzz/work/MA9/install（只复用现有MFA UI/host，逐项hash，不复用其Agent/interface/config）；AGENT=本轮build-root/build/agent/win-x64/dist/ma9-agent；BASE=本lane/build/user-test-slot-da22bf1；DESTINATION=BASE/MA9-preview；TASKS仅{多人运行时_数据自检,对决_隔离单槽定位测试}。sys.argv仅工具名，不zip。运行前确认DESTINATION不存在，避免原工具删除分支。
3. 资源、5项runtime数据必须来自da22bf1 lane；若OCR二进制模型为未跟踪缓存缺失，明确列出所需文件与主仓库现存受信本地缓存来源/哈希，再复制到本次包资源，不写lane源码、不联网下载，不用旧资源整体覆盖。除明示缓存外，资源逐文件与lane一致。
...
7. 包根新建TEST-BUILD.json：完整source HEAD、版本v0.0.0-slot-locate-da22bf1、构建脚本/解释器与exe哈希、构建根、源界面与包界面变换清单、marked-root阶段/私有绑定阶段、device_test=not_run、choose=false。绝不称整个包与source逐字节同一，因为interface明确过滤和child_exec变换。

必须实跑的包核验：
- PyInstaller内部递归代码对象比较（含数值常量，不忽略业务常量；仅允许源码目录前缀元数据差异）：至少runtime_action、ma9_agent.duel_slot_test、duel_slot_entry、duel_slot_selection、duel_lineup_slot、duel_vehicle_runtime、duel_vehicle_screen、vehicle_screen。旧verify仅4模块不够；可复用read_pyz/compare_code函数写本轮核验器，runtime_action用顶层key与agent/runtime_action.py。确保新module真实存在。不得只查字符串。
...
复用而非重跑：总控Agent268/tools30 exit0、05H新增pipeline+变更interface两项schema exit0，旧资源历史schema复用，native OCR18/18；本轮包界面经过过滤，应对最终interface及新增pipeline实跑schema（可用本轮tmp小资源目录+源validate_schema工具，旧未变资源复用），不能说全27重跑。不要跑npm或重建大分片。
...
## 本轮精确车型名修复包必读补充（02H）
构建源da22bf1已含05G1慢加载修复和05I共享唯一完整车型名匹配修复。必读20260924-05IR-exact-name-review/results.json及20260924-05I-exact-name/results.json、replay-matrix.json。总控268/tools30最终exit0复用，不重跑；schema输入无新增变更。当前精确目标第1槽Nevera仅定位，非Nevera R，不放行04或其他近名扩展测试。
旧02F user-test-slot-cf8815b与02G user-test-slot-f074163及其debug不动。新包唯一路径build/user-test-slot-da22bf1/MA9-preview，证据20260924-02H-da22bf1。不得调用旧包装脚本main写旧目录；可读取旧脚本函数或复制改成本轮私有路径再审输出位置。
模块核验必须8项：runtime_action、ma9_agent.duel_slot_test、duel_slot_entry、duel_slot_selection、duel_lineup_slot、duel_vehicle_runtime、duel_vehicle_screen、vehicle_screen。最后这个共享vehicle_screen是本轮真正变更点，不能沿用旧7项清单就称修复入包。递归比较代码和数值常量与当前lane逐项一致，源SHA也必须等于git对象da22bf1。
新增必做负对照：git show f074163fb77a163f76b73239a10842c03f07a90c:agent/ma9_agent/vehicle_screen.py 编译为同basename vehicle_screen.py，与包内ma9_agent.vehicle_screen比对，独立比较进程必须exit1且报告真实代码差异（不仅目录/文件名）。原runtime_action@1634旧对照也保留；允许总验证器assert期待非0后退出0，但各负对照子进程exit必须逐项记录。
另对包内提取的vehicle_screen代码做可控纯离线名字匹配探针（不得导入runtime_action/启动AgentServer；若需要相对models依赖按核验模块比对后加载受信源依赖并说明范围）：RIMAC+NEVERA完整名→Nevera，完整NEVERA R→Nevera R，目录反序同结果、低置信/重复精确/近似残缺名仍拒。若无法安全执行包内对象，可依赖完整递归一致性并运行源码纯匹配探针，必须明确“源码探针+包内等价证明”，不可冒充包内执行。
包interface.version显式设v0.0.0-slot-locate-da22bf1，仅改新包产物；TEST-BUILD记录第四项version变换。最终changed_keys应恰[agent,controller,task,version]，GUI仍只有自检+单槽定位，最终interface实跑schema。原正文只有三项变换的描述在本轮以本条四项为准。
OCR缓存4文件必须独立与声明的host来源逐一hash，不可用包内副本生成expected。同basename负对照和模块集合避免复用旧验包疏漏。
私有账号配置仍取05H user-request.template.json，但runtime_root必须改成本次新包绝对根；账号/slot1/目标/choosefalse其余不变。先原prepare完备性+标记，再私有config；不复制旧包garage/debug。包内试用说明写“完整车型名修复测试包”，别让用户跑旧包或宣称已找到目标。
无socket冒烟仍精确Usage/exit1，不允许GUI/ADB/真实Controller；完成后总控验包才交用户再试，not_run保持。报告F1只是离线构造的后缀丢失风险，不能写成真实设备反例，也不能据此宣称所有近名误认已解决。
